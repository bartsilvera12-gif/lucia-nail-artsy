...
interface PaywallProps {
  ...
  courseTitle: string;
  price: number;
  includedInMembership: boolean; // kept for compat, no longer used
  authenticated: boolean;
}

export function Paywall({ courseSlug, courseTitle, price, authenticated }: PaywallProps) {
  return (
    <div className="relative aspect-video w-full overflow-hidden rounded-xl border border-border bg-gradient-cream">
      <div className="absolute inset-0 flex flex-col items-center justify-center px-6 text-center">
        ...
        <p className="mt-2 max-w-md text-sm text-muted-foreground">
          Comprá el curso <span className="font-medium text-foreground">{courseTitle}</span> para acceder a todas las clases con acceso permanente.
        </p>

        <ul className="mt-5 grid gap-1 text-xs text-muted-foreground sm:grid-cols-2">
          {["Acceso inmediato", "Certificado al completar", "Bonos descargables", "Tuyo para siempre"].map((b) => (
